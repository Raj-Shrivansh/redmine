...
import type { RequestAuthContext } from "./context.js";
...
export type OAuthConfig = {
  redmineUrl: string;
  redmineClientId: string;
  redmineClientSecret: string;
  baseUrl: string;
  jwtSecret: string;
  jwtTtlSeconds: number;
  getEffectiveScopes: () => string[];
};

export class RedmineOAuthProxy {
  private readonly pendingAuth = new Map<string, PendingAuth>();
  private readonly sessions = new Map<string, RedmineSession>();
  ...
  async authenticateFromHeader(authorizationHeader?: string): Promise<RequestAuthContext | null> {
  ...
  private async handleCallback(req: Request, res: Response): Promise<void> {
    ...
    if (error) {
      ...
      if (pending.state) failure.searchParams.set("state", pending.state);
    ...
    if (pending.state) success.searchParams.set("state", pending.state);
  ...
  private verifyPkce(codeChallenge?: string, codeChallengeMethod?: string, codeVerifier?: string): boolean {
  ...
  private async issueJwt(session: RedmineSession): Promise<string> {
    return new SignJWT({
      sid: session.id,
  ...
  private async ensureFreshRedmineToken(session: RedmineSession): Promise<RedmineSession> {
  ...
  private async exchangeWithRedmine(args: {
