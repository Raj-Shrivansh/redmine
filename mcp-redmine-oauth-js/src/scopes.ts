...
export function declareScopes(...scopes: string[]): void {
  for (const scope of scopes) {
    registry.add(scope);
  }
}

export function setAllowedScopes(scopes: string[]): void {
...
export function getEffectiveScopes(): string[] {
  if (!allowedScopes) {
...
export function checkScope(grantedScopes: string[], ...required: string[]): string | null {
...
export function requireScopes<T extends unknown[]>(
  required: string[],
