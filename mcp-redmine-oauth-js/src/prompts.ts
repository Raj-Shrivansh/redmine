...
import { RedmineClient, RedmineForbiddenError, RedmineNotFoundError } from "./redmine-client.js";
...
type AnyServer = {
  prompt: (
    ...
    handler: (args: Record<string, unknown>) => Promise<{ messages: Array<{ role: "user"; content: { type: "text"; text: string } }> }>
  ) => void;
};
...
export function registerPrompts(server: AnyServer, redmine: RedmineClient): void {
  server.prompt(
    "summarize_ticket",
    "Generate a concise summary of a Redmine issue.",
    ...
    async (args) => {
      const run = requireScopes([VIEW_ISSUES], async (issueId: number) => {
        try {
          ...
          const journals = Array.isArray(issue.journals) ? (issue.journals as Record<string, unknown>[]) : [];
          const notes = journals
            .slice(-15)
            .map((entry) => {
              const text = String(entry.notes ?? "");
              if (!text) return null;
              const user = ((entry.user as Record<string, unknown>) ?? {}).name ?? "Unknown";
              return `- ${user}: ${text}`;
            })
...
- Status: ${((issue.status as Record<string, unknown>) ?? {}).name ?? "N/A"}
- Priority: ${((issue.priority as Record<string, unknown>) ?? {}).name ?? "N/A"}
- Assigned to: ${((issue.assigned_to as Record<string, unknown>) ?? {}).name ?? "Unassigned"}
...
3. Suggested next steps`;
  ...
  server.prompt(
    ...
    async (args) => {
      const run = requireScopes([VIEW_PROJECT], async (input: Record<string, unknown>) => {
        ...
        try {
          ...
          const trackers = Array.isArray(project.trackers) ? (project.trackers as Record<string, unknown>[]) : [];
          const categories = Array.isArray(project.issue_categories)
            ? (project.issue_categories as Record<string, unknown>[])
            : [];
