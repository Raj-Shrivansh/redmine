...
import { formatCreatedIssue, formatCreatedProject, formatIssue, formatIssueList, formatProject, formatRelations, formatSearchResults, formatTimeEntries, formatVersions, formatWikiPage } from "./formatters.js";
import { ADD_ISSUES, ADD_PROJECT, EDIT_ISSUES, EDIT_PROJECT, EDIT_WIKI_PAGES, RENAME_WIKI_PAGES, SEARCH_PROJECT, VIEW_ISSUES, VIEW_PROJECT, VIEW_TIME_ENTRIES, VIEW_WIKI_PAGES, requireScopes } from "./scopes.js";
import { RedmineForbiddenError, RedmineNotFoundError, RedmineValidationError } from "./redmine-client.js";
...
export function registerTools(server, redmine) {
    server.tool("get_issue_details", "Fetch full Redmine issue details including description, custom fields, and journals.", { issue_id: z.number().int() }, async (args) => {
    ...
    server.tool("search_issues", "Search Redmine issues by full-text query.", {
        ...
        const run = requireScopes([VIEW_ISSUES, SEARCH_PROJECT], async (input) => {
            const params = {
            ...
            if (input.open_issues_only !== false)
    ...
    server.tool("list_issues", "List Redmine issues with optional filters.", {
        project_id: z.string().optional(),
        ...
        const run = requireScopes([VIEW_ISSUES], async (input) => {
            const params = {
                offset: Number(input.offset ?? 0),
                limit: Number(input.limit ?? 25)
            };
            ...
            if (input.status_id)
                params.status_id = String(input.status_id);
            if (typeof input.tracker_id === "number")
                params.tracker_id = input.tracker_id;
            if (input.sort)
                params.sort = String(input.sort);
            try {
    ...
    server.tool("get_issue_relations", "Get issue relations (blocking, blocked-by, related).", { issue_id: z.number().int() }, async (args) => {
        const run = requireScopes([VIEW_ISSUES], async (issueId) => {
            try {
    ...
    server.tool("get_project_details", "Get detailed information about a Redmine project.", { project_id: z.string() }, async (args) => {
        const run = requireScopes([VIEW_PROJECT], async (projectId) => {
            try {
    ...
    server.tool("get_project_versions", "Get versions/milestones for a Redmine project.", { project_id: z.string() }, async (args) => {
        const run = requireScopes([VIEW_PROJECT], async (projectId) => {
            try {
    ...
    server.tool("list_time_entries", "List Redmine time entries with optional filters.", {
        project_id: z.string().optional(),
        ...
        const run = requireScopes([VIEW_TIME_ENTRIES], async (input) => {
            const params = {
                offset: Number(input.offset ?? 0),
                limit: Number(input.limit ?? 25)
            };
            if (input.project_id)
                params.project_id = String(input.project_id);
            if (input.user_id)
                params.user_id = String(input.user_id);
            if (input.from_date)
                params.from = String(input.from_date);
            if (input.to_date)
                params.to = String(input.to_date);
            try {
    ...
    server.tool("create_issue", "Create a new Redmine issue.", {
        project_id: z.string(),
        subject: z.string(),
        tracker_id: z.number().int().optional(),
        ...
        const run = requireScopes([ADD_ISSUES], async (input) => {
            const issueData = {
    ...
    server.tool("update_issue", "Update an existing Redmine issue.", {
        ...
        const run = requireScopes([EDIT_ISSUES], async (input) => {
            const issueData = {};
            for (const key of [
                "notes",
                "status_id",
    ...
    server.tool("create_project", "Create a new Redmine project.", {
        name: z.string(),
        identifier: z.string(),
        description: z.string().optional(),
        ...
        const run = requireScopes([ADD_PROJECT], async (input) => {
            const projectData = {
    ...
    server.tool("update_project", "Update an existing Redmine project.", {
        project_id: z.string(),
        ...
        const run = requireScopes([EDIT_PROJECT], async (input) => {
    ...
    server.tool("get_wiki_page", "Get a wiki page from a Redmine project.", {
        project_id: z.string(),
        page_title: z.string().default("Wiki")
    }, async (args) => {
        const run = requireScopes([VIEW_WIKI_PAGES], async (input) => {
            const projectId = String(input.project_id);
    ...
    server.tool("update_wiki_page", "Create or update a wiki page.", {
        project_id: z.string(),
        page_title: z.string(),
        content: z.string(),
        comments: z.string().optional()
    }, async (args) => {
        const run = requireScopes([EDIT_WIKI_PAGES], async (input) => {
            const projectId = String(input.project_id);
            const pageTitle = String(input.page_title);
            const wikiPage = { text: String(input.content) };
            if (input.comments)
                wikiPage.comments = String(input.comments);
            try {
    ...
    server.tool("rename_wiki_page", "Rename a wiki page in Redmine.", {
        project_id: z.string(),
        page_title: z.string(),
        new_title: z.string(),
        create_redirect: z.boolean().default(true)
    }, async (args) => {
        const run = requireScopes([RENAME_WIKI_PAGES], async (input) => {
            const projectId = String(input.project_id);
            const pageTitle = String(input.page_title);
            const newTitle = String(input.new_title);
            const createRedirect = input.create_redirect !== false;
            const wikiPage = { title: newTitle };
            if (!createRedirect)
                wikiPage.redirect_existing_links = 0;
            try {
                await redmine.put(`/projects/${projectId}/wiki/${pageTitle}.json`, getToken(), { wiki_page: wikiPage });
            }
            catch (error) {
